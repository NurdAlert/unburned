		...
		if (provider && provider->is_connected())
		{
			...
			if (local_player)
			{
				...
				for (const auto& client : client_list)
				{
					...
					tmp_cache.emplace_back<cached_player_t>({ client, player, player->life(), player->look(), nullptr, player->movement(), client->player_id()->character_name(), player->equipment()->asset()->item_name() });
...
int main()
{
	...
	while (msg.message != WM_QUIT)
	{
		...
		auto items = get_active_objects<interactable_item_t*>("InteractableItem");
